#ifndef RSI_GRAPHZONELIGHT_05072016
#define RSI_GRAPHZONELIGHT_05072016
...
namespace AASS{
	namespace RSI{
		...
		class GraphZoneLight : public bettergraph::SimpleGraph<ZoneLight, EdgeElementLight>{
		...
		public:
			typedef typename bettergraph::SimpleGraph<ZoneLight, EdgeElementLight>::GraphType GraphZoneLightType;
			typedef typename bettergraph::SimpleGraph<ZoneLight, EdgeElementLight>::Vertex VertexZoneLight;
			typedef typename bettergraph::SimpleGraph<ZoneLight, EdgeElementLight>::Edge EdgeZoneLight;
			typedef typename bettergraph::SimpleGraph<ZoneLight, EdgeElementLight>::VertexIterator IteratorVertexZoneLight;
			typedef typename bettergraph::SimpleGraph<ZoneLight, EdgeElementLight>::EdgeIterator IteratorEdgeZoneLight;

			GraphZoneLight(): _nb_of_unique(-1), _pca_classified(false), _sd_away_fom_mean_for_uniqueness(1), _threshold(0.25){};
			...
			void drawSimple(cv::Mat& m, const AASS::RSI::GraphZoneLight::VertexZoneLight& v, const cv::Scalar& color) const;
			///@brief draw map with all info on image
			void draw(cv::Mat& drawmat) const;
			void draw(cv::Mat& m, const bettergraph::SimpleGraph<ZoneLight, int>::Vertex& v, const cv::Scalar& color) const;
			...
			void drawEvaluation(cv::Mat& m, const bettergraph::SimpleGraph<ZoneLight, int>::Vertex& v, const cv::Scalar& color) const;
...
// 					}
			...
			///@brief update the contour of only one vertex and its neighbors
			void updateContours(VertexZoneLight& v){
				...
				for (boost::tie(out_i, out_end) = boost::out_edges(v, (*this)); 
					...
					(*this)[targ].updateContour();
				}
				(*this)[v].updateContour();
			}
			
			///@brief update all vertex contours SLOW!
			void updateContours(){
// 				std::pair<IteratorVertexZoneLight, IteratorVertexZoneLight> vp;
				auto vp = boost::vertices((*this));
				for(vp = boost::vertices((*this)) ; vp.first != vp.second;){
...
// 					std::cout << "PCA " <<  score_pca << " mean " << _mean_pca << " standar dev " << _sdeviation_pca << " pca " << (*this)[v].getPCADiff() << std::endl;
			...
			std::vector<int> removeRiples(){
				...
				std::vector<VertexZoneLight> vertex_zone;
				
				std::vector<VertexZoneLight> seen;
...
// 					std::cout << "Testing vertex " << std::endl;
					...
					for (boost::tie(out_i, out_end) = boost::out_edges(v, (*this)); 
						...
						EdgeZoneLight e_second = *out_i;
						VertexZoneLight targ = boost::target(e_second, (*this));
						VertexZoneLight src = boost::source(e_second, (*this));
...
// 									std::cout << "Removing a ripple because percentage is " << percentage << std::endl;
			...
			std::vector<ZoneComparedLight> compare(GraphZoneLight& target);
		...
		private:
			...
			*/
			void getAllNodeRemovedRipples(AASS::RSI::GraphZoneLight::VertexZoneLight& base_vertex, const std::deque< AASS::RSI::GraphZoneLight::VertexZoneLight >& top_vertex_visited, int dist = -1);
			void removeVertexWhilePreservingEdges(AASS::RSI::GraphZoneLight::VertexZoneLight& v, bool createUnBreakableLinks);
			void removeVertexWhilePreservingEdges(AASS::RSI::GraphZoneLight::VertexZoneLight& v, AASS::RSI::GraphZoneLight::VertexZoneLight& v_to_fuse_in, bool createUnBreakableLinks);
			
			///@brief Return true of the zone is ripple
			bool isRipple(const AASS::RSI::GraphZoneLight::VertexZoneLight& base_vertex, const AASS::RSI::GraphZoneLight::VertexZoneLight& might_be_ripple, bool updateContour = true) const;
			
			///@brief Check if the zone is a ripple of a neighborhood and removes it
			bool checkAndReplaceRipple(AASS::RSI::GraphZoneLight::VertexZoneLight& might_be_ripple, AASS::RSI::GraphZoneLight::VertexZoneLight& to_fuse_in);
...
// 					std::cout << "variance " << variance << std::endl;
			...
			void makeAllUnbreakableEdges(VertexZoneLight& base_vertex);
