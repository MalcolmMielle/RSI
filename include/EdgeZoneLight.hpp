#ifndef RSI_EDGEZONELIGHT_05072016
#define RSI_EDGEZONELIGHT_05072016

#include <boost/math/distributions/normal.hpp>

#include <ZoneLight.hpp>
#include "bettergraph/SimpleGraph.hpp"

namespace AASS{
	
	namespace RSI{
		
		class EdgeElementLight;
		
		struct ElementPairVertexLight{
			typedef typename bettergraph::SimpleGraph<ZoneLight, EdgeElementLight>::Vertex VertexZoneLight;
			VertexZoneLight from;
			VertexZoneLight toward;
			double score;
			///Minimum value between from and toward if ripple where added. It's -1 if not initialised and it's -2 if this is an original link from the first graph.
			double min_toward;
		};
		
		class EdgeElementLight{
		protected:
			double _diff;
			bool _breakable;
			bool _fromFuse;
			
			
			typedef typename bettergraph::SimpleGraph<ZoneLight, EdgeElementLight>::Vertex VertexZoneLight;
			std::vector<ElementPairVertexLight > _score_old;
			
			
		public:
			double min_toward;
			EdgeElementLight() : _diff(0), _breakable(true), _fromFuse(false){};
			
			EdgeElementLight(const EdgeElementLight& ed) : _diff(ed.getDiff()), _breakable(true){
				for (auto it = ed.getOldScore().begin() ; it != ed.getOldScore().end() ; ++it){
					_score_old.push_back(*it);
				}
			};
			bool wasRipple(){return _fromFuse;}
			size_t size(){return _score_old.size();}
			size_t size() const {return _score_old.size();}
			const std::vector<ElementPairVertexLight>& getOldScore() const {return _score_old;}
			double getDiff(){return _diff;}
			double getDiff() const {return _diff;}
			void setDiff(double diff){_diff = diff;}
			void makeUnbreakable(){_breakable = false;}
			bool canRemove() const {return _breakable;}
			
			double getOldScore(const VertexZoneLight& v, const VertexZoneLight& v_toward){
				for (auto it = _score_old.begin() ; it != _score_old.end() ; ++it){
					if(v == it->from && v_toward == it->toward){
						assert(it->score >=0);
						assert(it->score <= 255);
						return it->score;
					}
				}
				return -1;
			}
			
			void setOldScore(const VertexZoneLight& from, const VertexZoneLight& toward, double score){
				
				_fromFuse= true;
				
				ElementPairVertexLight el;
				el.from = from;
				el.toward = toward;
				el.score = score;
				bool found = false;
				//Find if node exist
// 				std::cout << "Yolo" << _score_old.size() << std::endl;
				
				//If the exact same link exist
				for (auto it = _score_old.begin() ; it != _score_old.end() ;){
					if(from == it->from && toward == it->toward){
// 						std::cout << "Set old score" << std::endl;
						found = true;
						el.score = it->score;
						el.min_toward = it->min_toward;
						//Keep the score closest to the start value. -2 means direct link
						if(el.min_toward <= score && el.min_toward != -2){
							el.min_toward = score;
						}
						auto it_cpy = it - 1;
						_score_old.erase(it);
						it = it_cpy +1;
					}
					else{
						++it;
					}
// 					std::cout << "done" << std::endl;
				}
// 				std::cout << "Push back" << std::endl;
				_score_old.push_back(el);
				
				//If we might create new links
				if(found == false){
					
					bool f_init = false;
					ElementPairVertexLight el;
					el.toward = toward;
					el.score = -1;
					el.min_toward = -1;
					for (auto it = _score_old.begin() ; it != _score_old.end() ; ++it){
						if(from == it->toward){
							ElementPairVertexLight el;
							el.from = it->from;
							el.min_toward = it->min_toward;
							el.min_toward = score;
							if(it->min_toward >= el.min_toward ){
								el.min_toward = it->min_toward;								
							}
							f_init = true;
						}
					}
					
					_score_old.push_back(el);
				}
				
			}
			
			//Test if need to be makeUnbreakable thanks to all value of zone fused in it
			bool shouldBeUnbreakable(double max_start, const VertexZoneLight& start, double max_end, const VertexZoneLight& end, double thresh){
// 				if(_score_old.empty() == false){
// 					auto it = _score_old.begin();
// 					double min = -1;
// 					for(it; it != _score_old.end() ; ++it){
// 						std::cout << it->from << " to " << it->toward << std::endl;
// // 						if( (it->score <= min || min == -1 ) && it->from != start && it->from != end && it->toward != start && it->toward != end){
// 						if( (it->min_toward <= min || min == -1 ) && ( (it->from == start && it->toward == end) || (it->from == end && it->toward == start) ) ){
// 							min = it->min_toward;
// 						}
// 					}
// 					std::cout << "min " << min << std::endl;
// 					if(min != -1){
// 						double max = std::max(max_start, max_end);
// 						std::cout << " GOOD max " << max << " min " << min << std::endl;
// 						if( min <= max - (max * thresh)){
// 							return true;
// 						}
// 					}
// 					//Original link here
// 					else if(min == -2){
// 						return false;
// 					}
// 					else{
// 						std::cout << "NOT FOUND :( " << start << " to " << end << std::endl;
// 						throw std::runtime_error("two vertex not found");
// 					}
// 				}
// 				return false;
// 				
				double max = std::max(max_start, max_end);
// 				std::cout << " GOOD max " << max << " min " << min_toward << std::endl;
				if(min_toward == -2){
					return false;
				}
				else if(min_toward <= max - (max * thresh)){
					return true;
				}
				return false;
				
			}
						
		};
		
		
		inline int getDirection(const EdgeElementLight& edg, const ZoneLight& src, const ZoneLight& targ, double thresh = 0){
			if(edg.getDiff() < thresh){
				if(src.getValue() > targ.getValue()){
					//Going down
					return 1;
				}
				else{
					//Going Up
					return 2;
				}
			}
			//Same value
			return 0;
			
		}
		
		
		inline std::ostream& operator<<(std::ostream& in, const EdgeElementLight &p){
			
			in << "d " << p.getDiff();
			return in;
			
		}
		
		inline std::istream& operator>>(std::istream& in, EdgeElementLight &p){
			char tmp; 
			in >> tmp;
			std::cout << tmp << " ";
			//TODO change to double
			int input;
			in >> input;
			std::cout << input << std::endl;
			p.setDiff(input);
			return in;
		}
		
		
	}
	
}


#endif