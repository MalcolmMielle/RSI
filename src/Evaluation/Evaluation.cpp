...
void AASS::RSI::Evaluation::compare(const cv::Mat& seg, const cv::Mat& GT_segmentation, double time, const std::string& file)
{
	...
	AASS::RSI::compare_images(GT_segmentation, seg, Regions);
...
void AASS::RSI::Evaluation::exportAll(const std::string& file_out)
{
...
void AASS::RSI::Evaluation::computeMeasures()
{
	int i;
...
void AASS::RSI::Evaluation::calculate()
{
	...
	_matthewCC = matthewCC<double>(tp, fp, tn, fn);
	_dor = DOR<double>(tp, fp, tn, fn);
	_accuracy = accuracy<double>(tp, fp, tn, fn);
	
	double mean_matthew = 0;
	for(auto it = _matthewCC_individual.begin() ; it != _matthewCC_individual.end(); ++it){
		mean_matthew = mean_matthew + *it;
	}
	mean_matthew = mean_matthew / _matthewCC_individual.size();
	
	std::cout << "Mathieu " << mean_matthew << " " << _matthewCC << std::endl;
	assert(mean_matthew = _matthewCC);
	
}


void AASS::RSI::Evaluation::compareImagesUnbiased(cv::Mat GT_segmentation_in, cv::Mat DuDe_segmentation_in)
{
...
void AASS::RSI::compare_images(cv::Mat GT_segmentation_in, cv::Mat DuDe_segmentation_in, AASS::RSI::results& res_out)
{
...
void AASS::RSI::extract_results(AASS::RSI::results& Regions, std::vector< float >& Precisions, std::vector< float >& Recalls, std::vector< float >& inverse_recall)
{
...
cv::Mat AASS::RSI::segment_Ground_Truth(cv::Mat GroundTruth_BW)
{
...
bool AASS::RSI::checkAllValue(const cv::Mat& mat_in, const std::vector< int >& all_vals)
{
